...
<c:forEach items="${followBarList}" var="bar" >
    <div class="followlist">
        <img src="/filePath/${bar.picName}" width="50px" height="50px"
             class="tabimg">
        <div class="followinfo">
            <span style="font-size: 15px;"><b>${bar.barName}</b></span>
            <span style="font-size: 10px;color: #ababab;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">经验 7/15</span>
        </div>
        <button class="fbtn" id="fbtn${bar.barId}" onclick="follow(${bar.barId})" >已关注</button>
    </div>
</c:forEach>
...
<script type="text/javascript">
	...
	window.onload=function(){
		$.ajax({
			url:"/user/selectFollowBar",
			type:"get",
			success:function (res) {
				// window.location="/user/toFollowBar";
			}
		})
	}
